        ...
        if (decodedToken.containsKey('Type') || decodedToken.containsKey('type')) {
          ...
          switch (roleType) {
            ...
            // Save admin ID to SharedPreferences
              ...
              if (admin != null) {
                emit(LoginModelViewSuccess(HomeScreenAdmin(admin: admin)));
              } else {
                emit(LoginModelViewError('Failed to fetch admin details.'));
              }
              break;
            case "2":
              emit(LoginModelViewSuccess(HomeScreenColla()));
              print('Collaborator login successful');
              break;
            default:
