...
class LoginModelViewCubit extends Cubit<LoginModelViewState> {
  ...
  void login(BuildContext context) async {
    ...
    if (token.isEmpty) {
      return emit(LoginModelViewError('Login failed.'));
    }
    ...
    if (!decodedToken.containsKey('Type') && !decodedToken.containsKey('type')) {
      ...
      return emit(LoginModelViewError('Role key "Type" not found in the token.'));
    }
    ...
    if (decodedToken.containsKey('Type') || decodedToken.containsKey('type')) {
      ...
      switch (roleType) {
        case "0":
          ...
          if (userId != null) {
            emit(LoginModelViewSuccess(OMHomeScreen(
              OMId: userId,
            )));
            print(decodedToken);
          } else {
        ...
        case "2":
          if (userId != null) {
            emit(LoginModelViewSuccess(ODHomeScreen(ODId: userId)));
            print(decodedToken);
          } else {
        ...
        case "3":
          if (userId != null) {
            emit(LoginModelViewSuccess(OAHomeScreen(OAId: userId)));
            print(decodedToken);
          } else {
        ...
        case "4":
          if (userId != null) {
            emit(LoginModelViewSuccess(OWHomeScreen(OWId: userId)));
            print(decodedToken);
          } else {
