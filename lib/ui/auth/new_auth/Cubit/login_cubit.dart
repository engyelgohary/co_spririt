...
import 'package:co_spirit/ui/home/od_home.dart';
...
class LoginModelViewCubit extends Cubit<CubitState> {
  AuthDataSource authDataSource;
  LoginModelViewCubit({required this.authDataSource}) : super(CubitInitialState());
    ...
    if (decodedToken.containsKey('Type') || decodedToken.containsKey('type')) {
      ...
      switch (roleType) {
        case "0":
          ...
          if (userId != null) {
            emit(CubitSuccessfulState(OMHomeScreen(
              OMId: userId,
            )));
            print(decodedToken);
          } else {
        ...
        case "2":
          if (userId != null) {
            emit(CubitSuccessfulState(ODHomeScreen(ODId: userId)));
            print(decodedToken);
          } else {
        ...
        case "3":
          if (userId != null) {
            emit(CubitSuccessfulState(OAHomeScreen(OAId: userId)));
            print(decodedToken);
          } else {
        ...
        case "4":
          if (userId != null) {
            emit(CubitSuccessfulState(OWHomeScreen(OWId: userId)));
            print(decodedToken);
          } else {
        ...
        case "5": //sc
          if (userId != null) {
            emit(CubitSuccessfulState(const SCHomePage()));
            print(decodedToken);
          } else {
            print('Role ID "nameid" not found for Opportunity Owner.');
            emit(CubitFailureState('Role ID "nameid" not found for Opportunity Owner.'));
        ...
        case "6": //sm
          if (userId != null) {
            ...
            emit(CubitSuccessfulState(const SMHomePage()));
            print(decodedToken);
          } else {
            print('Role ID "nameid" not found for Opportunity Owner.');
            emit(CubitFailureState('Role ID "nameid" not found for Opportunity Owner.'));
